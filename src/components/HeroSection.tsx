...
const HeroSection = () => {
  ...
  useEffect(() => {
    ...
    tl.fromTo(el.querySelector(".hero-tag")!, { y: 30, opacity: 0 }, { y: 0, opacity: 1, delay: 0.2 })
      .fromTo(el.querySelectorAll(".hero-line"), { y: 80, opacity: 0 }, { y: 0, opacity: 1, stagger: 0.15 }, "-=0.4")
      .fromTo(el.querySelector(".hero-sub")!, { y: 20, opacity: 0 }, { y: 0, opacity: 1, duration: 0.6 }, "-=0.3")
      .fromTo(el.querySelectorAll(".hero-btn"), { y: 20, opacity: 0 }, { y: 0, opacity: 1, stagger: 0.1, duration: 0.5 }, "-=0.3")
      .fromTo(el.querySelector(".hero-img")!, { scale: 0.9, opacity: 0 }, { scale: 1, opacity: 1, duration: 1 }, "-=1");
  ...
  return (
    <section id="home" ref={sectionRef} className="min-h-screen flex items-center section-padding pt-28 md:pt-32">
      <div className="max-w-7xl mx-auto w-full grid md:grid-cols-2 gap-12 items-center">
        <div>
          <div className="hero-tag inline-block bg-gold/10 text-gold-dark font-body text-sm font-semibold px-4 py-1.5 rounded-full mb-6" style={{ opacity: 0 }}>
          ...
          <h1 className="font-heading text-4xl md:text-5xl lg:text-6xl font-bold text-secondary leading-tight mb-6">
            <span className="hero-line block" style={{ opacity: 0 }}>Unlock the Stars,</span>
            <span className="hero-line block text-gold-gradient" style={{ opacity: 0 }}>Transform Your Life</span>
          </h1>
          <p className="hero-sub font-body text-muted-foreground text-lg max-w-lg mb-8 leading-relaxed" style={{ opacity: 0 }}>
        ...
        <div className="hero-img relative flex justify-center" style={{ opacity: 0 }}>
